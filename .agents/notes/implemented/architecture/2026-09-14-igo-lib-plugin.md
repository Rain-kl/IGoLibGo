...
13 张表使用 `igo_*` 前缀（不用平台 `w_`），按 `user_id` 隔离。TraceInt Cookie/GraphQL/签到客户端已接入会话、场馆、预约、协议、设置与四个任务节拍（`igo:tick`）。备份导入导出与 WebDAV 同步仍返回 501。Cookie / WebDAV 密码列目前明文存储。
...
- **代价与已知上限**：明日预约未接入 TraceInt WebSocket 排队通道，按设定触发时间直接 warmup/save。抢座节拍在 TaskService 不可用时只落库不轮询。若 TraceInt 协议按学校分叉到必须独立部署，再把传输层抽成可替换端口，而不是先拆插件。
