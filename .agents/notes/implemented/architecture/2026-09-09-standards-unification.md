...
1. **API 响应全面对齐 `api-design`**：重构 `backend/pkg/response`，提供标准 RESTful 状态码（200/201/204/400/404 等）与统一信封（`Response[T]`, `PagedResponse[T]`, `ErrorResponse`），同时在错误响应中继续透出 `error_msg`，确保老前端/调用方完全平滑兼容。
