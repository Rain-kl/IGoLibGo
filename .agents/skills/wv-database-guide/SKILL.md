...
日志/分析用途表的判定、三库回落与切换见 `wv-logstore` 技能。
...
- 日志/分析用途表必须同时在关系型主库建回落表并接入 `wv-logstore` 门面。
...
**不要**把不同日志域并入同一 channel。新日志表先按 `wv-logstore` skill 判定，再为本域建独立 writer。
