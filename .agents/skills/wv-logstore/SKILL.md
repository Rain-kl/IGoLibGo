...
开始前阅读根目录 `AGENTS.md`。DDL 用 `wv-database-guide`；高频写入队列用 `wv-database-guide`；切换任务用 `wv-new-async-task`。本技能只回答：**这张表是不是日志表，以及如何接入可切换的日志主库。**
...
**不要**做成日志表：用户、配置、任务执行、上传元数据、需要事务或强一致的业务实体。这些走主库业务插件 `repository`，不要进 `wv-logstore`。
