...
func (s *Service) CreatePipelineConfig(ctx context.Context, userID uint64, req do.CreatePipelineConfigRequest) (*do.PipelineConfigDTO, error) {
	...
	if req.AutoCheckin {
		if strings.TrimSpace(req.CheckinToken) != "" {
			checkinToken, checkinExp, err = s.resolveCheckinToken(ctx, userID, req.CheckinToken)
			if err != nil {
				return nil, err
			}
			// Verify checkin token
...
func (s *Service) RunPipeline(ctx context.Context, userID uint64, id string, overrideReq *do.RunPipelineRequest) (*do.PipelineRunResult, error) {
	...
	// 1. Check & apply override credentials if provided
	if overrideReq != nil {
	...
	_, err = cli.ListLibraries(ctx, tpl, row.Cookie)
	if err != nil {
		return &do.PipelineRunResult{
			Success:    false,
			ConfigID:   id,
	...
	// 3. If auto-checkin enabled, validate checkin token
	if row.AutoCheckin {
